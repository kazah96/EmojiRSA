...
function EncryptHTML(originalElement, outElement,errorElement, keyN)
{
    ...
    console.log("orig text: =" + original);



    var inputBigInt = StringToBigInt(SHA1(original));
    console.log("hash:" + SHA1(original));
...
function CheckDocument(TextElementId, SignatureElementId , KeyElementD, KeyElementN)
{
    var inputText = document.getElementById(TextElementId);
    var inputSignature = document.getElementById(SignatureElementId);
    var inputKeyD = document.getElementById(KeyElementD);
    var inputKeyN = document.getElementById(KeyElementN);
   
    var hash = SHA1(inputText.value);

    var DecodedSign = base100decodeFromStringToBigInt(inputSignature.value);
    var Decrypted = RSA.decrypt(DecodedSign,inputKeyD.value, inputKeyN.value);
    var outMessage = BigIntToString(Decrypted);
    
    var OutHash = outMessage.split('').reverse().join('');

    if(hash == OutHash)
        inputText.setAttribute("class", "rightText");
    else
        inputText.setAttribute("class", "wrongText");



    console.log("Hash decrypted " + OutHash + "\nHashGenerated " + hash);
